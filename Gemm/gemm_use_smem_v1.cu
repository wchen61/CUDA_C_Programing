    ...
    for (int k = 0; k < K; k += BK) {
        ...
        for (int tk = 0; tk < BK; tk++) {
            ...
            for (int tm = 0; tm < TM; tm++) {
                ...
                float r_a = tileA[a_smem_m][tk];
                
                // avg_Gflops  12088 -> 12231
                #pragma unroll
                /*for (int tn = 0; tn < TN; tn++) {
                    int b_smem_n = tx * TN + tn;
                    float r_b = tileB[tk][b_smem_n];
                    r_c[tm][tn] += r_a * r_b;
                }*/
                for (int tn = 0; tn < TN; tn += 4) {
                    int b_smem_n = tx * TN + tn;
                    float4 r_b = *reinterpret_cast<float4*>(&tileB[tk][b_smem_n]);
                    r_c[tm][tn    ] += r_a * r_b.x;
                    r_c[tm][tn + 1] += r_a * r_b.y;
                    r_c[tm][tn + 2] += r_a * r_b.z;
                    r_c[tm][tn + 3] += r_a * r_b.w;
                }
            }
        }
        __syncthreads();
